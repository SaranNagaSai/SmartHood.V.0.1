...
import {
    ...
    RefreshCw, ChevronRight, FileSpreadsheet, Loader2, CheckCircle
} from 'lucide-react';
...
const AdminUserStats = () => {
    ...
    const handleExportExcel = async () => {
        ...
        try {
            const token = localStorage.getItem('adminToken');
            const res = await fetch(`${API_URL}/admin/users/${id}/export-excel`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${token}` }
            });
            const json = await res.json();
            if (json.success) {
                setExportMsg({ type: 'success', text: `Report sent to ${json.sentTo.length} admins!` });
            } else {
                setExportMsg({ type: 'error', text: json.message || 'Export failed' });
            }
        } catch (err) {
            console.error('Export failed', err);
            setExportMsg({ type: 'error', text: 'Failed to export. Try again.' });
        }
        setExporting(false);
                        ...
                        <div className="flex items-center gap-3">
                            {exportMsg && (
                                <span className={`text-xs px-3 py-1.5 rounded-full font-medium ${
                                    exportMsg.type === 'success' ? 'bg-green-500/20 text-green-200' : 'bg-red-500/20 text-red-200'
                                }`}>
                                    {exportMsg.type === 'success' && <CheckCircle size={12} className="inline mr-1" />}
                                    {exportMsg.text}
                                </span>
                            )}
                            <button
                                ...
                                disabled={exporting}
                                className="flex items-center gap-2 px-4 py-2 bg-white/15 hover:bg-white/25 backdrop-blur-sm rounded-xl text-sm font-bold transition border border-white/20 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {exporting ? (
                                ...
                                {exporting ? 'Sending...' : 'Create Excel'}
                            </button>
                        </div>
                    </div>
